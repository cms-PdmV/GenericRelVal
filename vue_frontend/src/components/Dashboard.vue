<template>
  <div>
    ...
    <v-card raised class="page-card">
      ...
      <ul>
        <li v-for="(info, worker) in submissionWorkers" :key="worker">
          Thread "{{ worker }}" is
          {{
            info.job_name
              ? "working on " + info.job_name + " for " + info.job_time + "s"
              : "not busy"
          }}
        </li>
      </ul>
      ...
      <ul>
        <li v-for="name in submissionQueue" :key="name">
          <a :href="'relvals?prepid=' + name" title="Show this request">{{
            name
          }}</a>
          is waiting in queue
        </li>
      ...
      <h3 class="mt-3" v-if="role('administrator')">
        Settings ({{ Object.keys(settings).length }})
      </h3>
      <small v-if="role('administrator')">
        <ul>
          <li v-for="setting in settings" :key="setting._id">
            {{ setting._id }}:
      ...
      <h3 class="mt-3" v-if="role('administrator')">
        Locked objects ({{ Object.keys(locks).length }})
      </h3>
      <small v-if="role('administrator')">
        <ul>
          <li
            v-for="(info, lock) in locks"
            :key="lock"
            :style="info ? 'color: red; font-weight: bold;' : ''"
          >
            {{ lock }}: {{ info }}
          </li>
        </ul>
