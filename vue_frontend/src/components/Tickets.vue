      ...
      <div style="flex: 1 1 auto">
        ...
        >
          <template v-slot:item._actions="{ item }">
            <div class="actions">
              <a
                :href="'tickets/edit?prepid=' + item.prepid"
                v-if="role('manager')"
                title="Edit ticket"
                >Edit</a
              >
              <a
                ...
                v-if="
                  role('manager') &&
                  (item.status == 'new' ||
                    !item.created_relvals ||
                    item.created_relvals.length == 0)
                "
                title="Delete ticket"
                >Delete</a
              >
              <a
                :href="'tickets/edit?clone=' + item.prepid"
                v-if="role('manager')"
                title="Clone ticket"
                >Clone</a
              >
              ...
              >
              <a
                :href="'api/tickets/run_the_matrix/' + item.prepid"
                target="_blank"
                title="Show a runTheMatrix.py command for this ticket"
                >runTheMatrix.py</a
              >
            </div>
          </template>
          <template v-slot:item.prepid="{ item }">
            <a
              :href="'tickets?prepid=' + item.prepid"
              title="Show only this ticket"
              >{{ item.prepid }}</a
            >
          </template>
          <template v-slot:item.status="{ item }">
            <a
              ...
              :title="'Show all tickets with status ' + item.status"
              >{{ item.status }}</a
            >
          </template>
          <template v-slot:item.matrix="{ item }">
            <a
              :href="'tickets?matrix=' + item.matrix"
              :title="'Show all tickets with ' + item.matrix + ' matrix'"
              >{{ item.matrix }}</a
            >
          </template>
          <template v-slot:item.sample_tag="{ item }">
            <a
              :href="'tickets?sample_tag=' + item.sample_tag"
              :title="
                'Show all tickets with ' + item.sample_tag + ' sample tag'
              "
              >{{ item.sample_tag }}</a
            >
          </template>
          <template v-slot:item.label="{ item }">
            <a
              :href="'tickets?label=' + item.label"
              :title="'Show all tickets with ' + item.label + ' label'"
              >{{ item.label }}</a
            >
          </template>
          ...
          <template v-slot:item.workflow_ids="{ item }">
            <span v-if="item.workflow_ids.length"
              >{{ item.workflow_ids.length }} workflows:
              <small>{{ item.workflow_ids.join(", ") }}</small></span
            >
          </template>
          <template v-slot:item.cmssw_release="{ item }">
            <a
              :href="
                'tickets?cmssw_release=*' + parseRelease(item.cmssw_release)
              "
              :title="
                'Show all tickets with ' + parseRelease(item.cmssw_release)
              "
            >
            ...
            <small v-if="item.scram_arch.length">
              <br />
              <a
                :href="'tickets?scram_arch=' + item.scram_arch"
                :title="'Show all tickets with ' + item.scram_arch"
                >{{ item.scram_arch }}</a
              >
            </small>
          </template>
          <template v-slot:item.batch_name="{ item }">
            <a
              :href="'tickets?batch_name=' + item.batch_name"
              :title="'Show all tickets with ' + item.batch_name"
              >{{ item.batch_name }}</a
            >
          </template>
          <template v-slot:item.created_relvals="{ item }">
            ...
            <ul>
              <li v-for="relval in item.created_relvals" :key="relval">
                <a
                  :href="'relvals?prepid=' + relval"
                  :title="'Open ' + relval + ' request'"
                  >{{ relval }}</a
                >
              </li>
            </ul>
          ...
          <template v-slot:item.command="{ item }">
            <pre v-if="item.command && item.command.length" class="notes">{{
              item.command
            }}</pre>
          </template>
          <template v-slot:item._created="{ item }">
            Created by
            <a
              :href="'tickets?created_by=' + item.history[0].user"
              :title="'Show all tickets created by ' + item.history[0].user"
              >{{ item.history[0].user }}</a
            >
            on {{ niceDate(item.history[0].time) }}
          </template>
          <template v-slot:item.gpu="{ item }">
            ...
            <ul v-if="item.gpu.requires != 'forbidden'">
              <li v-if="item.gpu.gpu_memory">
                GPUMemory: {{ item.gpu.gpu_memory }}
              </li>
              <li v-if="item.gpu.cuda_capabilities">
                CUDACapabilities: {{ item.gpu.cuda_capabilities }}
              </li>
              <li v-if="item.gpu.cuda_runtime">
                CUDARuntime: {{ item.gpu.cuda_runtime }}
              </li>
              <li v-if="item.gpu.gpu_name">GPUName: {{ item.gpu.gpu_name }}</li>
              <li v-if="item.gpu.cuda_driver_version">
                CUDADriverVersion: {{ item.gpu.cuda_driver_version }}
              </li>
              <li v-if="item.gpu.cuda_runtime_version">
                CUDARuntimeVersion: {{ item.gpu.cuda_runtime_version }}
              </li>
            </ul>
          </template>
          <template v-slot:item.recycle_input_of="{ item }">
            <a
              :href="'tickets?recycle_input_of=' + item.recycle_input_of"
              :title="
                'Show all tickets that recycle ' +
                item.recycle_input_of +
                ' input'
              "
              >{{ item.recycle_input_of }}</a
            >
          </template>
        </v-data-table>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          dbName: "events_factor",
          displayName: "Events Factor",
          visible: 0,
          sortable: true,
        },
        ...
        {
          dbName: "recycle_input_of",
          displayName: "Recycle input of",
          visible: 0,
          sortable: true,
        },
        {
          dbName: "rewrite_gt_string",
          displayName: "Rewrite GT String",
          visible: 0,
          sortable: true,
        },
        {
  ...
  methods: {
    fetchObjects() {
      ...
      axios
        .get("api/search?db_name=tickets" + queryParams)
        .then((response) => {
    ...
    showDeleteDialog: function (ticket) {
      ...
      this.dialog.description =
        "Are you sure you want to delete " + ticket.prepid + " ticket?";
      this.dialog.ok = function () {
        component.loading = true;
        axios
          .delete("api/tickets/delete", { data: { prepid: ticket.prepid } })
          .then(() => {
            component.clearDialog();
    ...
    showCreateRelValsDialog: function (ticket) {
      ...
      this.dialog.ok = function () {
        ...
        axios
          .post(
            "api/tickets/create_relvals",
            { prepid: ticket.prepid },
            { timeout: 180000 }
          )
          .then(() => {
