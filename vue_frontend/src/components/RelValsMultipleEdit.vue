      ...
      <table v-if="editingObject">
        <tr>
          ...
          <td>
            <input
              type="number"
              v-model="editingObject.cpu_cores"
              min="1"
              max="8"
            />
          </td>
        ...
        <tr>
          ...
          <td>
            <input
              type="text"
              v-model="editingObject.sample_tag"
        ...
        <tr>
          ...
          <td>
            <input type="number" v-model="editingObject.size_per_event" />kB
          </td>
        </tr>
          ...
          <td>
            <div v-for="(step, index) in editingObject.steps" :key="index">
              ...
              <table v-if="!step.deleted">
                <tr>
                  <td>Name</td>
                  <td><input type="text" v-model="step.name" /></td>
                </tr>
                <tr>
                  <td>CMSSW Release</td>
                  <td><input type="text" v-model="step.cmssw_release" /></td>
                </tr>
                <tr>
                  <td>SCRAM Arch</td>
                  <td>
                ...
                <tr v-if="index == 0 && step.step_type != 'input'">
                  ...
                  <td><input type="text" v-model="step.events_per_lumi" /></td>
                </tr>
                <tr v-if="step.step_type != 'input'">
                  <td>Keep output</td>
                  <td><input type="checkbox" v-model="step.keep_output" /></td>
                </tr>
                <tr v-if="index != 0">
                  <td>Lumis per job</td>
                  <td><input type="text" v-model="step.lumis_per_job" /></td>
                </tr>
                <template v-if="index == 0">
                  <tr>
                    <td>Dataset</td>
                    <td><input type="text" v-model="step.input.dataset" /></td>
                  </tr>
                  <tr>
                    <td>Label</td>
                    <td><input type="text" v-model="step.input.label" /></td>
                  </tr>
                  <tr>
                ...
                <tr>
                  <td>--beamspot</td>
                  <td><input type="text" v-model="step.driver.beamspot" /></td>
                </tr>
                <tr>
                  <td>--conditions</td>
                  <td>
                    <input type="text" v-model="step.driver.conditions" />
                  </td>
                </tr>
                <tr>
                  <td>--customise</td>
                  <td><input type="text" v-model="step.driver.customise" /></td>
                </tr>
                <tr>
                ...
                <tr>
                  <td>--datatier</td>
                  <td><input type="text" v-model="step.driver.datatier" /></td>
                </tr>
                <tr>
                  <td>--era</td>
                  <td><input type="text" v-model="step.driver.era" /></td>
                </tr>
                <tr>
                  <td>--eventcontent</td>
                  <td>
                    <input type="text" v-model="step.driver.eventcontent" />
                  </td>
                </tr>
                <tr>
                  <td>--filetype</td>
                  <td><input type="text" v-model="step.driver.filetype" /></td>
                </tr>
                <tr>
                  <td>--geometry</td>
                  <td><input type="text" v-model="step.driver.geometry" /></td>
                </tr>
                <tr>
                  <td>--hltProcess</td>
                  <td>
                    <input type="text" v-model="step.driver.hltProcess" />
                  </td>
                </tr>
                <tr>
                  <td>--nStreams</td>
                  <td><input type="text" v-model="step.driver.nStreams" /></td>
                </tr>
                <tr>
                  <td>--pileup</td>
                  <td><input type="text" v-model="step.driver.pileup" /></td>
                </tr>
                <tr>
                  <td>--pileup_input</td>
                  <td>
                    <input type="text" v-model="step.driver.pileup_input" />
                  </td>
                </tr>
                <tr>
                  <td>--process</td>
                  <td><input type="text" v-model="step.driver.process" /></td>
                </tr>
                <tr>
                  <td>--relval</td>
                  <td><input type="text" v-model="step.driver.relval" /></td>
                </tr>
                <tr>
                ...
                <tr>
                  <td>--step</td>
                  <td><input type="text" v-model="step.driver.step" /></td>
                </tr>
                <tr>
                  ...
                  <td>
                    <input
                      ...
                      @click="
                        step.driver.data = !Boolean(step.driver.data);
                  ...
                  <td>
                    <table>
                      ...
                      <tr>
                        <td>CUDA Runtime</td>
                        <td>
                          <input type="text" v-model="step.gpu.cuda_runtime" />
                        </td>
                      </tr>
                      <tr>
                        <td>GPU Name</td>
                        <td>
                          <input type="text" v-model="step.gpu.gpu_name" />
                        </td>
                      </tr>
                      <tr>
              ...
              >
              <span v-if="step.deleted"> Deleted </span>
              <hr />
            </div>
          </td>
        </tr>
        <tr>
          ...
          <td>
            <input type="number" v-model="editingObject.time_per_event" />s
          </td>
        </tr>
        <tr>
          <td>Workflow Name</td>
          ...
          <template v-if="key != 'steps'">
            <li>
              ...
              <v-btn
                ...
                class="ml-1 mb-1"
                color="error"
                @click="editingObject[key] = originalEditingObject[key]"
                >Remove edit</v-btn
              >
            </li>
          </template>
            ...
            <div v-for="(step, index) in value" :key="index">
              <template v-if="!step.deleted">
                <div v-for="(stepValue, stepKey) in step" :key="stepKey">
                  <template v-if="['driver', 'input', 'gpu'].includes(stepKey)">
                    <li
                      v-for="(driverValue, driverKey) in stepValue"
                      :key="driverKey"
                    >
                      <b>{{ driverKey }}</b> in <b>Step {{ index + 1 }}</b> will
                      be set to <b>{{ driverValue }}</b>
                      <v-btn
                        small
                        class="ml-1 mb-1"
                        color="error"
                        @click="
                          editingObject.steps[index][stepKey][driverKey] =
                            originalEditingObject.steps[index][stepKey][
                              driverKey
                            ]
                        "
                        >Remove edit</v-btn
                  ...
                  <template
                    v-if="
                      !['deleted', 'driver', 'input', 'gpu'].includes(stepKey)
                    "
                  >
                    <li>
                      <b>{{ stepKey }}</b> in <b>Step {{ index + 1 }}</b> will
                      be set to <b>{{ stepValue }}</b>
                      <v-btn
                        small
                        class="ml-1 mb-1"
                        color="error"
                        @click="
                          editingObject.steps[index][stepKey] =
                            originalEditingObject.steps[index][stepKey]
                        "
                        >Remove edit</v-btn
                      >
              ...
              <li v-if="step.deleted">
                ...
                <v-btn
                  ...
                  class="ml-1 mb-1"
                  color="error"
                  @click="editingObject.steps[index].deleted = false"
                  >Remove edit</v-btn
                >
              </li>
            </div>
...
export default {
  ...
  created() {
    ...
    let component = this;
    axios
      .get("api/relvals/get_editable/" + this.prepids.join(","))
      .then((response) => {
        ...
        component.$set(
          component,
          "editingObject",
          component.makeCopy(editingObject)
        );
        component.$set(
          component,
          "originalEditingObject",
  ...
  methods: {
    ...
    prepareObjectForSaving: function (obj) {
      ...
      for (let step of obj.steps) {
        step.input.lumisection = step.input.lumisection
          ? JSON.parse(step.input.lumisection)
          : {};
    ...
    makeEditingObject: function (relvals) {
      ...
      // Steps
      editingObject.steps = Array(
        Math.min(...relvals.map((x) => x.steps.length))
      );
      for (let i = 0; i < editingObject.steps.length; i++) {
        editingObject.steps[i] = {
          deleted: false,
          driver: {},
          input: {},
          gpu: {},
        ...
        if (["driver", "input", "gpu"].includes(key)) {
          for (let i = 0; i < editingObject.steps.length; i++) {
            for (let driverKey of Object.keys(relvals[0].steps[i][key])) {
              editingObject.steps[i][key][driverKey] = this.getCommonValue(
                relvals,
                (r) => r.steps[i][key][driverKey]
              );
            }
          }
        } else {
          for (let i = 0; i < editingObject.steps.length; i++) {
            editingObject.steps[i][key] = this.getCommonValue(
              relvals,
              (r) => r.steps[i][key]
            );
          }
    ...
    save: function () {
                ...
                for (let stepKey of Object.keys(diffStep)) {
                  if (["driver", "input", "gpu"].includes(stepKey)) {
                    for (let driverKey of Object.keys(diffStep[stepKey])) {
                      objStep[stepKey][driverKey] =
                        diffStep[stepKey][driverKey];
                    }
      ...
      httpRequest
        .then((response) => {
          ...
          window.location =
            "relvals?prepid=" +
            response.data.response.map((x) => x.prepid).join(",");
        })
        .catch((error) => {
          component.loading = false;
          component.showError(
            "Error saving relvals",
            component.getError(error)
          );
        });
    },
