<template>
  <div style="height: calc(100vh - 118px); overflow: auto">
      ...
      <div style="flex: 1 1 auto">
        ...
        >
          <template v-slot:item._actions="{ item }">
            <div class="actions">
              <a
                :href="'relvals/edit?prepid=' + item.prepid"
                v-if="role('manager')"
                >Edit</a
              >
              <a
                @click="deleteRelVals([item])"
                v-if="item.status == 'new' && role('manager')"
                >Delete</a
              >
              <a
              ...
              >
              <a
                :href="'api/relvals/get_dict/' + item.prepid"
                title="Show JSON dictionary for ReqMgr2"
                >Job dict<b
                  v-if="Object.keys(item.job_dict_overwrite).length > 0"
                  style="color: red"
                >
                  !</b
                ></a
              ...
              <a
                ...
                v-if="role('manager') && item.status != 'done'"
                title="Move to next status"
                >Next</a
              >
              <a
                @click="updateWorkflows([item])"
                v-if="
              ...
              <a
                target="_blank"
                :href="'https://cms-pdmv.cern.ch/stats?prepid=' + item.prepid"
                v-if="
          ...
          <template v-slot:item.notes="{ item }">
            <pre
              ...
              v-html="sanitize(item.notes)"
              class="notes"
              v-linkified
            ></pre>
          </template>
          <template v-slot:item.fragment="{ item }">
            <pre v-if="item.fragment.length" class="fragment">{{
              item.fragment
            }}</pre>
          </template>
          ...
          <template v-slot:item._workflow="{ item }">
            <a :href="'relvals?workflow_id=' + item.workflow_id">{{
              item.workflow_id
            }}</a>
            <span v-if="item.workflow_name">({{ item.workflow_name }})</span>
          </template>
            ...
            <ol>
              <li v-for="workflow in item.workflows" :key="workflow.name">
                <a
                  v-if="!isDev"
                  target="_blank"
                  title="Open workflow in ReqMgr2"
                  :href="
                    'https://cmsweb.cern.ch/reqmgr2/fetch?rid=' + workflow.name
                  "
                  >{{ workflow.name }}</a
                >&nbsp;
                <a
                  ...
                  title="Open workflow in ReqMgr2"
                  :href="
                    'https://cmsweb-testbed.cern.ch/reqmgr2/fetch?rid=' +
                    workflow.name
                  "
                  >{{ workflow.name }}</a
                >&nbsp;
                <template v-if="!isDev">
                  ...
                  <a
                    target="_blank"
                    title="Open workflow in Stats2"
                    :href="
                      'https://cms-pdmv.cern.ch/stats?workflow_name=' +
                      workflow.name
                    "
                    >Stats2</a
                  >&nbsp;
                ...
                <template
                  v-if="
                    ...
                    workflow.status_history.length > 0
                  "
                >
                  <small> status:</small>
                  {{
                    workflow.status_history[workflow.status_history.length - 1]
                      .status
                  }}
                </template>
                <ul class="zebra-datasets">
                  <li
                    v-for="dataset in workflow.output_datasets"
                    :key="dataset.name"
                  >
                    <div>
                      <div class="gray-bar">
                        <div
                          :style="'width: ' + dataset.completed + '%;'"
                          :class="'bar ' + dataset.type.toLowerCase() + '-bar'"
                        ></div>
                      </div>
                      ...
                      <small>type:</small>
                      <b :class="dataset.type.toLowerCase() + '-type'">{{
                        dataset.type
                      }}</b>
                      <br />
                      <small style="letter-spacing: -0.1px"
                        ><a
                          target="_blank"
                          title="Open dataset in DAS"
                          :href="makeDASLink(dataset.name)"
                          >{{ dataset.name }}</a
                        ></small
                      >
                    </div>
          ...
          <template v-slot:item.output_datasets="{ item }">
            <ul>
              <li v-for="dataset in item.output_datasets" :key="dataset">
                <a
                  target="_blank"
                  title="Open dataset in DAS"
                  :href="makeDASLink(dataset)"
                  >{{ dataset }}</a
                >
              </li>
            </ul>
          ...
          <template v-slot:item.cmssw_release="{ item }">
            ...
            >
              {{ parseRelease(item.cmssw_release) }}
              <b
                v-if="item.cmssw_release && item.cmssw_release[0] == '/'"
                style="color: red"
              >
                !</b
              >
              ...
              <li v-for="(step, index) in item.steps" :key="index">
                <template v-if="stepSteps(step)">
                  ...
                  <ul v-if="step.gpu.requires != 'forbidden'">
                    <li v-if="step.gpu.gpu_memory">
                      GPUMemory: {{ step.gpu.gpu_memory }} MB
                    </li>
                    <li v-if="step.gpu.cuda_capabilities.length">
                      CUDACapabilities:
                      {{ step.gpu.cuda_capabilities.join(",") }}
                    </li>
                    <li v-if="step.gpu.cuda_runtime">
                      CUDARuntime: {{ step.gpu.cuda_runtime }}
                    </li>
                    <li v-if="step.gpu.gpu_name">
                      GPUName: {{ step.gpu.gpu_name }}
                    </li>
                    <li v-if="step.gpu.cuda_driver_version">
                      CUDADriverVersion: {{ step.gpu.cuda_driver_version }}
                    </li>
                    <li v-if="step.gpu.cuda_runtime_version">
                      CUDARuntimeVersion: {{ step.gpu.cuda_runtime_version }}
                    </li>
                  </ul>
                </template>
              </li>
    ...
    <v-dialog v-model="dialog.visible" max-width="50%">
      <v-card>
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          dbName: "output_datasets",
          displayName: "Output Datasets",
          visible: 0,
        },
        {
          dbName: "sample_tag",
        ...
        {
          dbName: "size_per_event",
          displayName: "Size per Event",
          visible: 0,
          sortable: true,
        },
        { dbName: "steps", displayName: "Steps", visible: 0 },
        {
          dbName: "time_per_event",
          displayName: "Time per Event",
          visible: 0,
          sortable: true,
        },
        {
          dbName: "workflows",
          displayName: "Workflows (jobs in ReqMgr2)",
          visible: 0,
        },
      ],
      headers: [],
  ...
  methods: {
    fetchObjects() {
      ...
      axios
        .get("api/search?db_name=relvals" + queryParams)
        .then((response) => {
          component.dataItems = response.data.response.results.map(function (
            x
          ) {
            x._actions = undefined;
            return x;
          });
          component.dataItems.forEach((item) => {
            item.workflows = item.workflows.filter(
              (x) => !x.type || x.type.toLowerCase() != "resubmission"
            );
            for (let workflow of item.workflows) {
              if (workflow.output_datasets) {
                workflow.output_datasets.forEach((ds) => {
                  ...
                  ds.completed = (
                    workflow.total_events > 0
                      ? (ds.events / workflow.total_events) * 100
                      : 0
                  ).toFixed(2);
    ...
    showDeleteDialog: function (relval) {
      ...
      this.dialog.description =
        "Are you sure you want to delete " + relval.prepid + " ticket?";
      this.dialog.ok = function () {
        component.loading = true;
        axios
          .delete("api/relvals/delete", { data: { prepid: relval.prepid } })
          .then(() => {
            component.clearDialog();
    ...
    deleteRelVals: function (relvals) {
      ...
      this.dialog.ok = function () {
        component.loading = true;
        axios
          .delete("api/relvals/delete", { data: relvals.slice() })
          .then(() => {
            component.clearDialog();
            component.fetchObjects();
    ...
    nextStatus: function (relvals) {
      ...
      const submit = function () {
        ...
        component.loading = true;
        axios
          .post("api/relvals/next_status", relvals.slice(), { timeout: 0 })
          .then(() => {
            component.fetchObjects();
            component.selectedItems = [];
          })
    ...
    previousStatus: function (relvals) {
      ...
      this.dialog.ok = function () {
        component.loading = true;
        axios
          .post("api/relvals/previous_status", relvals.slice())
          .then(() => {
            component.clearDialog();
            component.fetchObjects();
            component.selectedItems = [];
    ...
    updateWorkflows: function (relvals) {
      let component = this;
      this.loading = true;
      let relvalOnlyIds = relvals.map((x) =>
        Object({ _id: x["_id"], prepid: x["prepid"] })
      );
      axios
        .post("api/relvals/update_workflows", relvalOnlyIds)
        .then(() => {
          component.fetchObjects();
          component.selectedItems = [];
        })
    ...
    openPmpMany: function (relvals) {
      let prepids = relvals.map((x) => x["prepid"]);
      let url =
        "https://cms-pdmv.cern.ch/pmp/historical?r=" + prepids.join(",");
